    ...
    *,
    ...
    if not hasattr(agent, "_codex_session") or agent._codex_session is None:
        ...
        def _on_codex_event(note: dict) -> None:
            ...
            # Newer sessions emit the same lifecycle through their native
            # tool_progress_callback; keep on_event only as a compatibility
            # fallback so a tool start is never rendered twice.
            session = getattr(agent, "_codex_session", None)
            if getattr(session, "_tool_progress_callback", None) is not None:
                return
